...
## 五、Builder 输出模板规范（v0.3.6）

7 章节结构，每一项带 LLM 引导示例：

| 章节 | 新增/强化 | 关键约束 |
|------|----------|---------|
| 1. 项目概述 | 角色定义 | 至少2类用户角色 |
| 2. 功能需求 | 用户故事格式 + 输入输出 + 模块依赖 | `作为<角色>我想<目标>以便<价值>` |
| 3. 非功能需求 | 错误处理子章节 | 每模块至少一个异常路径 |
| 4. 技术架构概要 | ✅ 新增 | 技术栈 + 选择理由 |
| 5. 数据模型概要 | 字段表 + 关系 | `字段名 \| 类型 \| 必填 \| 说明` |
| 6. MVP 范围 | 拆分 MVP / 不做 | 每项附原因 |
| 7. 术语表 | ✅ 新增 | 消除歧义 |

核心原则：每个 `##` 章节必须填充，不留空、不写"待定"。
